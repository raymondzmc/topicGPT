"""
StackOverflow TopicGPT Pipeline - Together AI (Llama 3.1-8B)
...
All steps use Together AI's Llama 3.1-8B-Instruct-Turbo model.
...
if not os.getenv("TOGETHER_AI_API_KEY"):
    raise ValueError("TOGETHER_AI_API_KEY not found in environment. Add it to your .env file.")
...
PRICE_INPUT = 0.18   # $0.18 per 1M input tokens
PRICE_OUTPUT = 0.18  # $0.18 per 1M output tokens
...
# Output directory
OUTPUT_DIR = "data/output/stackoverflow_together"
...
class TokenTracker:
    ...
    def __init__(self, model: str, price_input: float, price_output: float):
        self.model = model
    ...
    def get_step_cost(self, step_name: str) -> float:
        ...
        input_cost = (step.get("prompt_tokens", 0) / 1_000_000) * self.price_input
        output_cost = (step.get("completion_tokens", 0) / 1_000_000) * self.price_output
    ...
    def get_totals(self) -> dict:
        ...
        total_cost = (total_prompt / 1_000_000) * self.price_input + \
                     (total_completion / 1_000_000) * self.price_output
    ...
    def print_final_summary(self):
        ...
        print("=" * 70)
        print(f"\nModel: {self.model}")
        print(f"Pricing: ${self.price_input}/1M input, ${self.price_output}/1M output")
        print()
        
        # Per-step breakdown
        print(f"{'Step':<20} {'Prompt':>12} {'Completion':>12} {'Cost':>10} {'Time':>10}")
        print("-" * 70)
        ...
        for step_name in self.steps:
            ...
            cost = self.get_step_cost(step_name)
            duration = self.get_step_duration(step_name)
            print(f"{step_name:<20} {step['prompt_tokens']:>12,} {step['completion_tokens']:>12,} ${cost:>9.4f} {duration:>8.1f}s")
        
        print("-" * 70)
        print(f"{'TOTAL':<20} {totals['prompt_tokens']:>12,} {totals['completion_tokens']:>12,} ${totals['total_cost']:>9.4f} {totals['total_duration_seconds']:>8.1f}s")
        print("=" * 70)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "model": self.model,
            "pricing": {
                "input_per_1m": self.price_input,
                "output_per_1m": self.price_output,
            },
            "steps": {
                name: {
                    **step,
...
def _patched_init(self, api, model, host=None):
    ...
    if shared_client is not None and api == API and model == MODEL:
...
def main():
    ...
    # Initialize tracker
    tracker = TokenTracker(MODEL, PRICE_INPUT, PRICE_OUTPUT)
    ...
    print("=" * 70)
    print("StackOverflow TopicGPT Pipeline - Together AI")
    print("=" * 70)
    print(f"\nModel: {MODEL}")
    ...
    shared_client = APIClient(api=API, model=MODEL)
    
    # ========================================
    # Step 1: Topic Generation
    # ========================================
    print("\n" + "=" * 70)
    print("STEP 1/5: Topic Generation")
    print("=" * 70)
    ...
    # ========================================
    print("\n" + "=" * 70)
    print("STEP 2/5: Topic Refinement")
    print("=" * 70)
    ...
    # ========================================
    print("\n" + "=" * 70)
    print("STEP 3/5: Topic Assignment")
    print("=" * 70)
    
    tracker.start_step("Assignment")
    ...
    # ========================================
    # Step 4: Topic Correction
    # ========================================
    print("\n" + "=" * 70)
    print("STEP 4/5: Topic Correction")
    print("=" * 70)
    
    tracker.start_step("Correction")
